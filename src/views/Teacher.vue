...
<div>
    ...
    <el-table-column label="操作" width="185">
      <template v-slot="scope">
        <el-button type="success" @click="lookCourse(scope.row)" size="mini">
          查看授课<i class="el-icon-zoom-in"></i>
        </el-button>
      </template>
    </el-table-column>
  </el-table>
  ...
  <el-dialog title="课程信息" :visible.sync="dialogCourse" width="50%">
    <el-table :data="courses" border stripe>
      ...
      <el-table-column label="操作" width="185">
        <template v-slot="scope">
          <el-button type="success" @click="lookStudent(scope.row)" size="mini">
            查看此课程的学生<i class="el-icon-zoom-in"></i>
          </el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  <el-dialog title="学生信息" :visible.sync="dialogStudent" width="40%">
    ...
    <div slot="footer" class="dialog-footer">
      <el-button icon="el-icon-close" type="info" @click="dialogStudent = false">取 消</el-button>
      <el-button icon="el-icon-circle-check" type="primary" @click="dialogStudent = false">确 定</el-button>
    </div>
  </el-dialog>
</div>
</template>
...
export default {
  ...
  methods: {
    lookStudent(row) {
      request.get('course/studentsOfCourse', {
        params: {id: row.id}
      }).then(
          res => {
            if (res.code === '200') {
    ...
    lookCourse(row) {
      request.get('user/course', {
        params: {
          current: this.current, pageSize: this.pageSize, id: row.id
        }
      }).then(
          res => {
            if (res.code === '200') {
    ...
    load() {
      ...
      request.get('user/find', {
        params: {
          current: this.current, pageSize: this.pageSize, role: "ROLE_TEACHER"
        }
      }).then(
          res => {
            if (res.code !== '401'){
